...
def editContent(content_id):
    ...
    if item.user_id != login_session['user_id']:
        ...
        "to edit this content. Please create your own content.')"
...
@app.route('/content/<int:content_id>/delete',
           methods=['GET', 'POST'])
def deleteContent(content_id):
    content = session.query(Content).all()
    contentToDelete = session.query(Content).filter_by(id=content_id).one()
    item = contentToDelete
    if 'username' not in login_session:
        return redirect('/login')
    if item.user_id != login_session['user_id']:
        return "<script>function myFunction() {alert('You're not authorized'"
        " to delete this content source. Please create your own source'"
...
def newRecommendations(content_id):
    if 'username' not in login_session:
        return redirect('/login')
    content = session.query(Content).filter_by(id=content_id).one()
    if login_session['user_id'] != content.user_id:
        return "<script>function myFunction() {alert('You are not'"
        " authorized to add content recommendations to this content'"
        " source. Please create your own content source in order to'"
...
def editRecommendations(content_id, rec_id):
    ...
    if login_session['user_id'] != content.user_id:
        return "<script>function myFunction() {alert('You are not'"
        " authorized to edit recommendations for this content source.'"
        " Please create your own content in order to edit items.'"
        ");}</script><body onload='myFunction()''>"
    if request.method == 'POST':
        if request.form['name']:
